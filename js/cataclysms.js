...
export function triggerCataclysm(pJS) {
    ...
    switch(choice) {
        case 'Supernova': for(const p of pJS.particles.array) { p.vx = (seededRandom()-0.5)*50; p.vy = (seededRandom()-0.5)*50; }; regen(2500); break;
        case 'Phase Shift': pJS.particles.array.forEach(p => p.opacity.value = 0.1); pJS.fn.particlesRefresh(); regen(3000); break;
        case 'Glitch Storm': let glitches = 0; const glitchInterval = setInterval(() => { glitches++; for(const p of pJS.particles.array) { p.x += (seededRandom()-0.5)*20; p.y += (seededRandom()-0.5)*20; if(seededRandom() > 0.95) p.shape = ['circle', 'triangle', 'edge', 'star'][Math.floor(seededRandom()*4)]; } if(glitches > 50) { clearInterval(glitchInterval); regen(1000); } }, 50); addActiveInterval(glitchInterval); break;
        ...
        case 'Overgrowth': pJS.particles.line_linked.enable = true; let growths=0; const overgrowInterval = setInterval(() => { growths++; for(let i=0; i<5; i++) { if(pJS.particles.array.length < pJS.particles.number.value_max) { const p = pJS.particles.array[Math.floor(seededRandom()*pJS.particles.array.length)]; pJS.fn.modes.pushParticles(1, {x:p.x,y:p.y}); }} if(growths > 60) { clearInterval(overgrowInterval); regen(2000); } }, 50); addActiveInterval(overgrowInterval); break;
        ...
        case 'False Vacuum Decay': let decayBubble = {x: pJS.canvas.w/2, y: pJS.canvas.h/2, r: 0}; let decayInterval = setInterval(() => { decayBubble.r += pJS.canvas.w * 0.01; pJS.particles.array.forEach(p => { const dx = p.x-decayBubble.x; const dy = p.y-decayBubble.y; if(dx*dx+dy*dy < decayBubble.r*decayBubble.r) { p.vx*=-0.5; p.vy*=-0.5; p.color = {rgb:{r:255,g:100,b:255}};} }); if (decayBubble.r > pJS.canvas.w) { clearInterval(decayInterval); regen(1000); } }, 50); addActiveInterval(decayInterval); break;
        ...
        case 'UnseenGibbering': let gibber = 0; const gibberInterval = setInterval(() => { gibber++; for(let i=pJS.particles.array.length-1; i>=0; i--) { const p = pJS.particles.array[i]; p.vx += (seededRandom()-0.5)*2; p.vy += (seededRandom()-0.5)*2; if (seededRandom() > 0.99) pJS.particles.array.splice(i, 1); } if(gibber > 150) { clearInterval(gibberInterval); regen(1000); } }, 20); addActiveInterval(gibberInterval); break;
        ...
        case 'CoreEruption': pJS.particles.array.forEach(p => { const dx=p.x-pJS.canvas.w/2, dy=p.y-pJS.canvas.h/2, dist=Math.sqrt(dx*dx+dy*dy)||1; p.vx = dx/dist * (20 + seededRandom()*20); p.vy = dy/dist * (20 + seededRandom()*20); p.color = {rgb:{r:255,g:50,b:0}}; }); regen(3000); break;
        case 'DeepFreeze': physics.friction = 1.05; pJS.particles.array.forEach(p => { p.color = {rgb:{r:200,g:220,b:255}}; }); setTimeout(() => { pJS.particles.array.forEach(p => { p.vx += (seededRandom()-0.5)*5; p.vy += (seededRandom()-0.5)*5; if(p.radius > 1) p.radius *= 0.5; }); }, 2500); regen(3500); break;
        case 'HiveCollapse': let collapses = 0; const collapseInterval = setInterval(() => { collapses++; for(let i=pJS.particles.array.length-1; i>=0; i--) { const p = pJS.particles.array[i]; p.vx += (pJS.canvas.w/2 - p.x) * 0.005; p.vy += (pJS.canvas.h/2 - p.y) * 0.005; if(seededRandom() > 0.9) pJS.particles.array.splice(i,1); } if(collapses > 120) { clearInterval(collapseInterval); regen(1000); } }, 30); addActiveInterval(collapseInterval); break;
        case 'ForbiddenRitual': const ritualCenter = {x: pJS.canvas.w/2, y: pJS.canvas.h/2}; let ritual = 0; const ritualInterval = setInterval(() => { ritual++; pJS.particles.array.forEach(p => { const dx = ritualCenter.x - p.x, dy = ritualCenter.y - p.y, distSq=dx*dx+dy*dy; p.vx += dx * 0.003; p.vy += dy * 0.003; p.vx *= 0.98; p.vy *= 0.98; if (distSq < 2500) {p.vx=0;p.vy=0;} p.color = {rgb:{r:200,g:50,b:255}}; }); if(ritual > 200) { pJS.particles.array.forEach(p => { p.vx = (seededRandom()-0.5)*50; p.vy = (seededRandom()-0.5)*50; }); clearInterval(ritualInterval); regen(2000); } }, 20); addActiveInterval(ritualInterval); break;
        ...
        case 'BenthicStorm': let storm = 0; const stormInterval = setInterval(() => { storm++; physics.friction = 0.7; pJS.particles.array.forEach(p => { p.vx += (seededRandom()-0.5)*2; p.vy += (seededRandom()-0.5)*2; }); if(storm > 100) { clearInterval(stormInterval); regen(1000); } }, 50); addActiveInterval(stormInterval); break;
        ...
        case 'Decomposition': let decomp = 0; const decompInterval = setInterval(() => { decomp++; const arr=pJS.particles.array; for(let d=0;d<5&&arr.length>0;d++){arr.pop();} if(decomp > 100) { clearInterval(decompInterval); regen(1000); } }, 50); addActiveInterval(decompInterval); break;
        case 'Shattering': pJS.particles.array.forEach(p => { p.vx = (seededRandom()-0.5)*15; p.vy = (seededRandom()-0.5)*15; if (p.radius > 1) p.radius *= 0.5; }); regen(2000); break;
        ...
        case 'BigRip': let rip = 0; const ripInterval = setInterval(() => { rip++; for(let i=pJS.particles.array.length-1; i>=0; i--) { const p = pJS.particles.array[i]; p.vx *= 1.05; p.vy *= 1.05; p.radius *= 0.99; if(p.radius < 0.1) pJS.particles.array.splice(i, 1); } if(rip > 150 || pJS.particles.array.length === 0) { clearInterval(ripInterval); regen(1000); } }, 20); addActiveInterval(ripInterval); break;
