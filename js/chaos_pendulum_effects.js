...
export class ChaosPendulum {
    ...
    // Double pendulum physics (Runge-Kutta 4th order for stability)
    _stepDoublePendulum(p, dt, gx, gy) {
        const g = Math.sqrt(gx * gx + gy * gy);
        const gAngle = Math.atan2(gy, gx) - Math.PI / 2;

        // Effective angles relative to gravity direction
        ...
        const denom1 = l1 * (M - m2 * cosD * cosD);
        const denom2 = l2 * (M - m2 * cosD * cosD);

        if (Math.abs(denom1) < 1e-10 || Math.abs(denom2) < 1e-10) return;

        const a1a = (-g * M * Math.sin(a1 - gAngle) -
                     m2 * g * Math.sin(a1 - 2 * a2 + gAngle) -  // correction term
                     2 * sinD * m2 * (a2v * a2v * l2 + a1v * a1v * l1 * cosD)) /
                    (l1 * (2 * M - m2 * (1 + Math.cos(2 * delta))));

        const a2a = (2 * sinD * (a1v * a1v * l1 * M +
                     g * M * Math.cos(a1 - gAngle) +
                     a2v * a2v * l2 * m2 * cosD)) /
                    (l2 * (2 * M - m2 * (1 + Math.cos(2 * delta))));

        // Semi-implicit Euler (fast, good enough for visual purposes)
        p.a1v += a1a * dt;
        p.a2v += a2a * dt;
        p.a1v *= p.damping;
        p.a2v *= p.damping;
    ...
    _getEndpoints(p, cx, cy) {
        ...
        let x3 = x2, y3 = y2;
        if (p.isTriple) {
            x3 = x2 + p.l3 * Math.sin(p.a3);
            y3 = y2 + p.l3 * Math.cos(p.a3);
        }
    ...
    update(system) {
        ...
        for (let s = 0; s < subSteps; s++) {
            for (const p of this.pendulums) {
                this._stepDoublePendulum(p, dt, gx, gy);
                // Triple pendulum: treat arm3 as driven by arm2's endpoint
                if (p.isTriple) {
                    const simpleG = this._gravityStrength;
                    const a3a = (-simpleG / p.l3) * Math.sin(p.a3 - this._gravityAngle + Math.PI / 2);
                    p.a3v += a3a * dt * 0.5;
                    p.a3v *= p.damping;
                    p.a3 += p.a3v * dt;
                }
        ...
        if (this.mode === 3) {
            const speed = this.pendulums.length > 0 ?
                Math.abs(this.pendulums[0].a1v) + Math.abs(this.pendulums[0].a2v) : 0;
    ...
    draw(ctx, system) {
        ...
        for (let i = 0; i < this.pendulums.length; i++) {
            ...
            const cx = p.pivotX * w;
            const cy = p.pivotY * h;
            const ep = this._getEndpoints(p, cx, cy);
            const tipX = p.isTriple ? ep.x3 : ep.x2;
            const tipY = p.isTriple ? ep.y3 : ep.y2;
            ...
            switch (this.mode) {
                ...
                case 2: { // Lissajous hybrid - connect endpoints with parametric curves
                    ...
                    tc.beginPath();
                    tc.moveTo(tipX, tipY);
                    tc.quadraticCurveTo(lx, ly, ep.x1, ep.y1);
                ...
                case 3: { // Gravity paint - thick brush strokes
                    ...
                    tc.fillStyle = `hsla(${trailHue}, ${sat}%, ${light}%, ${0.3 + pressure * 0.4})`;
                    tc.fill();
                    break;
                }
                ...
                case 4: // Fractal fern - thin delicate trails
                    ...
                    tc.fillStyle = `hsla(${120 + p.hueOffset * 0.3}, 60%, 50%, 0.6)`;
                    tc.fill();
                    // Also trace mid-joint
                    tc.beginPath();
                    tc.arc(ep.x2, ep.y2, 0.5, 0, TAU);
                    tc.fillStyle = `hsla(${120 + p.hueOffset * 0.3}, 40%, 40%, 0.3)`;
                    tc.fill();
                    break;
                ...
                case 5: // Phase portrait
                    ...
                    pc.fillStyle = `hsla(${(p.hueOffset + this.tick * 0.2) % 360}, 80%, 60%, 0.5)`;
                    pc.fill();
                    break;
            }
        }
        ...
        for (const p of this.pendulums) {
            ...
            ctx.beginPath();
            ctx.moveTo(cx, cy);
            ctx.lineTo(ep.x1, ep.y1);
            ctx.lineTo(ep.x2, ep.y2);
            if (p.isTriple) ctx.lineTo(ep.x3, ep.y3);
            ctx.stroke();

            // Joint dots
            ctx.fillStyle = `hsla(${p.trailHue}, 60%, 80%, 0.5)`;
            ctx.beginPath();
            ctx.arc(ep.x1, ep.y1, 3, 0, TAU);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(ep.x2, ep.y2, p.isTriple ? 2 : 4, 0, TAU);
            ctx.fill();
            if (p.isTriple) {
                ctx.beginPath();
                ctx.arc(ep.x3, ep.y3, 4, 0, TAU);
                ctx.fill();
            }
        }
        ctx.restore();
