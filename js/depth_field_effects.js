...
export class DepthField {
    ...
    configure(rng, palette) {
        ...
        for (let layer = 0; layer < this._numLayers; layer++) {
            const depth = layer / (this._numLayers - 1);
            const parallax = 0.1 + depth * 0.9;
            ...
            for (let i = 0; i < objectCount; i++) {
                layerObjects.push(this._createObject(rng, W, H, layer, depth));
            }
    ...
    _createObject(rng, W, H, layer, depth) {
        ...
        if (this.mode === 0) {
            obj.type = Math.floor(rng() * 3);
            if (layer === 0) obj.brightness *= 0.4;
        } else if (this.mode === 1) {
            obj.type = Math.floor(rng() * 3);
            obj.bobAmount *= 1.5;
        } else if (this.mode === 2) {
            obj.fallSpeed = 0.3 + rng() * 1.5 * (0.5 + depth);
            obj.type = Math.floor(rng() * 5);
        } else if (this.mode === 3) {
            obj.runeType = Math.floor(rng() * 8);
    ...
    update(mx, my, isClicking) {
        ...
        for (let layer = 0; layer < this._objects.length; layer++) {
            for (const obj of this._objects[layer]) {
                ...
                if (this.mode === 2) {
                    ...
                    if (obj.baseY > H + 50) {
                        obj.baseY = -50;
                        obj.baseX = _prand(this.tick * 13 + layer * 41 + obj.phase * 100) * W * 1.4 - W * 0.2;
                    }
                }
        ...
        // Click burst — more particles for better impact
        if (isClicking && !this._wasClicking) {
            for (let i = 0; i < 12; i++) {
                const burst = this._burstPool.length > 0 ? this._burstPool.pop() : {};
                const angle = (i / 12) * TAU + this._pr() * 0.3;
                burst.x = mx; burst.y = my;
                burst.vx = Math.cos(angle) * (2 + this._pr() * 4);
                burst.vy = Math.sin(angle) * (2 + this._pr() * 4);
                burst.life = 1.0;
                burst.size = 2 + this._pr() * 4;
                burst.hue = (this.hue + this._pr() * 40) % 360;
                burst.layer = Math.floor(this._pr() * this._numLayers);
        ...
        for (let i = this._bursts.length - 1; i >= 0; i--) {
            ...
            b.life -= 0.02;
            if (b.life <= 0) {
                if (this._burstPool.length < 30) this._burstPool.push(b);
    ...
    draw(ctx, system) {
        ...
        for (let layer = 0; layer < this._objects.length; layer++) {
            ...
            for (const obj of this._objects[layer]) {
                ...
                if (this.mode === 0) this._drawCosmic(ctx, obj, x, y, hue, alpha);
                else if (this.mode === 1) this._drawUndersea(ctx, obj, x, y, hue, alpha);
                else if (this.mode === 2) this._drawGeoRain(ctx, obj, x, y, hue, alpha);
                else if (this.mode === 3) this._drawRune(ctx, obj, x, y, hue, alpha);
                else if (this.mode === 4) this._drawNebula(ctx, obj, x, y, hue, alpha);
                else if (this.mode === 5) this._drawCrystalCavern(ctx, obj, x, y, hue, alpha);
            }
            ...
            for (const b of this._bursts) {
                ...
                ctx.beginPath();
                ctx.arc(b.x + px, b.y + py, b.size * b.life, 0, TAU);
                ctx.fill();
            }
        }
    ...
    _drawCosmic(ctx, obj, x, y, hue, alpha) {
        if (obj.type === 0) {
            const size = obj.size;
    ...
    _drawUndersea(ctx, obj, x, y, hue, alpha) {
        if (obj.type === 0) {
            ctx.fillStyle = `hsla(${hue},50%,60%,${alpha * 0.6})`;
            ctx.beginPath();
            ctx.ellipse(x, y, obj.size, obj.size * 0.6, 0, Math.PI, 0);
            ctx.fill();
            ctx.strokeStyle = `hsla(${hue},40%,55%,${alpha * 0.3})`;
            ctx.lineWidth = 0.5;
            for (let t = 0; t < 4; t++) {
                const tx = x - obj.size * 0.6 + (t + 1) * (obj.size * 1.2 / 5);
                ctx.beginPath();
                ctx.moveTo(tx, y);
                ctx.quadraticCurveTo(tx + Math.sin(this.tick * 0.02 + t) * 5,
                    y + obj.size, tx, y + obj.size * 1.5);
                ctx.stroke();
            }
        } else if (obj.type === 1) {
            ctx.strokeStyle = `hsla(${hue + 40},30%,80%,${alpha * 0.5})`;
            ctx.lineWidth = 0.5;
            ctx.beginPath(); ctx.arc(x, y, obj.size, 0, TAU); ctx.stroke();
    ...
    _drawGeoRain(ctx, obj, x, y, hue, alpha) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(obj.rotation);
        const size = obj.size;
    ...
    _drawRune(ctx, obj, x, y, hue, alpha) {
        ...
        if (pulse > 0.5) {
            ...
            ctx.beginPath(); ctx.arc(x, y, size * 3, 0, TAU); ctx.fill();
        }
    ...
    _drawNebula(ctx, obj, x, y, hue, alpha) {
        const size = obj.size * 2;
        ctx.fillStyle = `hsla(${hue},60%,50%,${alpha * 0.3})`;
        ctx.beginPath(); ctx.arc(x, y, size, 0, TAU); ctx.fill();
    }

    _drawCrystalCavern(ctx, obj, x, y, hue, alpha) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(obj.rotation);
        const size = obj.size;
