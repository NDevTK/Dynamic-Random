...
// --- Individual Effect Drawing Functions ---
...
function drawPulsars(ctx) {
    activeEffects.pulsars.forEach(p => {
        p.angle += 0.05;
        ctx.fillStyle = 'white';
        ctx.beginPath();
...
function drawBlackHoles(ctx) {
    activeEffects.blackHoles.forEach(h => {
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(h.x, h.y, h.eventHorizon, 0, 2 * Math.PI);
        ctx.fill();
        const grad = ctx.createRadialGradient(h.x, h.y, h.eventHorizon, h.x, h.y, h.eventHorizon + 5);
...
function drawWhiteHoles(ctx, tick) {
    activeEffects.whiteHoles.forEach(h => {
        ctx.fillStyle = `rgba(255,255,255,${0.5 + 0.5 * Math.sin(tick * 0.1)})`;
        ctx.beginPath();
        ctx.arc(h.x, h.y, 10, 0, 2 * Math.PI);
        ctx.fill();
    });
}

function drawWormholes(ctx) {
    activeEffects.wormholes.forEach((w, i) => {
        w.life--;
        if (w.life <= 0) {
            activeEffects.wormholes.splice(i, 1);
            return;
        }
        ctx.strokeStyle = `rgba(150, 100, 255, ${w.life / 120})`;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(w.entry.x, w.entry.y, 15, 0, 2 * Math.PI);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(w.exit.x, w.exit.y, 15, 0, 2 * Math.PI);
        ctx.stroke();
    });
}

function drawPhaseZones(ctx) {
    activeEffects.phaseZones.forEach((z, i) => {
        z.life--;
        if (z.life <= 0) {
            activeEffects.phaseZones.splice(i, 1);
        }
        ctx.strokeStyle = `rgba(180, 200, 255, ${0.1 + (z.life / z.maxLife) * 0.3})`;
        ctx.setLineDash([15, 10]);
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(z.x, z.y, Math.sqrt(z.radiusSq), 0, 2 * Math.PI);
        ctx.stroke();
        ctx.setLineDash([]);
    });
}

function drawStasisFields(ctx, tick) {
    activeEffects.stasisFields.forEach((f, i) => {
        f.life--;
        if (f.life <= 0) {
            activeEffects.stasisFields.splice(i, 1);
        }
        ctx.strokeStyle = `rgba(255, 255, 150, ${0.1 + (f.life / f.maxLife) * 0.4})`;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(f.x, f.y, f.r * (0.95 + 0.05 * Math.sin(tick * 0.1)), 0, 2 * Math.PI);
        ctx.stroke();
    });
}
...
function drawQuasars(ctx) {
    activeEffects.quasars.forEach(q => {
        ...
        if (q.isFiring) {
            ...
            ctx.beginPath();
            ctx.moveTo(q.x, q.y);
            ctx.lineTo(q.x + Math.cos(q.angle) * 1000, q.y + Math.sin(q.angle) * 1000);
            ctx.stroke();
        }
    });
}

function drawCosmicRifts(ctx) {
    activeEffects.cosmicRifts.forEach(r => {
        ctx.strokeStyle = `rgba(200, 150, 255, 0.5)`;
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(r.x1, r.y1, r.width, 0, 2 * Math.PI);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(r.x2, r.y2, r.width, 0, 2 * Math.PI);
        ctx.stroke();
    });
}
...
function drawSupergiantStars(ctx, tick) {
    activeEffects.supergiantStars.forEach(s => {
        ctx.fillStyle = `rgba(255, 200, 180, ${0.5 + 0.5 * Math.sin(tick * 0.05)})`;
        ctx.beginPath();
...
function drawCrystallineFields(ctx) {
    activeEffects.crystallineFields.forEach(f => {
        ctx.strokeStyle = `rgba(180, 220, 255, 0.1)`;
        ctx.lineWidth = 1;
        ctx.beginPath();
...
function drawTemporalRifts(ctx) {
    activeEffects.temporalRifts.forEach(r => {
        ctx.strokeStyle = `rgba(255, 100, 100, ${0.1 + (r.life / r.maxLife) * 0.2})`;
        ctx.setLineDash([5, 5]);
        ctx.lineWidth = 1;
        ctx.beginPath();
...
function drawNegativeSpaces(ctx) {
    activeEffects.negativeSpaces.forEach(s => {
        ctx.fillStyle = `#000`;
        ctx.beginPath();
        ctx.arc(s.x, s.y, s.radius, 0, 2 * Math.PI);
        ctx.fill();
...
function drawParticleAccelerators(ctx) {
    activeEffects.particleAccelerators.forEach(a => {
...
function drawCoralConnections(ctx) {
    activeEffects.coralConnections.forEach(conn => {
        if (conn.p1 && conn.p2) {
            ctx.strokeStyle = 'rgba(200, 255, 220, 0.5)';
            ctx.lineWidth = conn.p1.radius / 2;
            ctx.beginPath();
...
function drawCosmicRivers(ctx) {
    activeEffects.cosmicRivers.forEach(river => {
        ...
        ctx.bezierCurveTo(river.cx1, river.cy1, river.cx2, river.cy2, river.x2, river.y2);
        ctx.stroke();
...
export function drawEffects(ctx) {
    ...
    drawPulsars(ctx);
    drawBlackHoles(ctx);
    drawWhiteHoles(ctx, tick);
    drawWormholes(ctx);
    drawPhaseZones(ctx);
    drawStasisFields(ctx, tick);
    drawCosmicWebs(ctx);
    drawQuasars(ctx);
    drawCosmicRifts(ctx);
    drawIonClouds(ctx);
    drawSupergiantStars(ctx, tick);
    drawCrystallineFields(ctx);
    drawTemporalRifts(ctx);
    drawNegativeSpaces(ctx);
    drawParticleAccelerators(ctx);
    drawSpacetimeFoam(ctx);
