...
export function handleClickPower(powerName, pJS, worldMouse) {
    switch(powerName) {
        ...
        case 'connectConstellation': { const inRange = []; for (const p of pJS.particles.array) { const dx=p.x-worldMouse.x, dy=p.y-worldMouse.y; if (p.life && dx*dx+dy*dy < 22500) inRange.push(p); } if(inRange.length > 1) { const p1 = inRange[Math.floor(seededRandom()*inRange.length)]; let p2 = inRange[Math.floor(seededRandom()*inRange.length)]; let attempts = 0; while(p1===p2 && attempts < 20) { p2 = inRange[Math.floor(seededRandom()*inRange.length)]; attempts++; } if(p1 !== p2) activeEffects.silkThreads.push({p1: p1, p2: p2, life: 400}); } } break;
